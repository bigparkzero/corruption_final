...
public class IndicatorElementAdder : MonoBehaviour
{
    ...
    public void AddElement(CompassElementObject element)
    {
        ...
        rectTransform.sizeDelta = element.indicatorAdderSize; // 크기 설정 (필요에 따라 조정)
    ...
    public void UpdateElements()
    {
        for (int i = 0; i < compassElements.Count; i++)
        {
            ...
            if (element != null && i < uiElements.Count)
            {
                ...
                // UI 요소의 위치를 업데이트
                uiElements[i].GetComponent<RectTransform>().anchoredPosition = uiPosition;
            }
        }
    }
}
